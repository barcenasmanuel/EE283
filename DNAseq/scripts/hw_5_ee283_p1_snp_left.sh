...
# Create FASTA dictionary if it doesn't exist
if [[ ! -f ${ref%.*}.dict ]]; then
...
for genotype in $genotypes; do
    dedupBam="${outputDir}/${genotype}.dedup.bam"
    metricsFile="${outputDir}/${genotype}_marked_dup_metrics.txt"

    # Check if deduplicated BAM and metrics files exist
    if [[ ! -f $dedupBam || ! -f $metricsFile ]]; then
    ...
    if [[ ! -f ${outputDir}/${genotype}.g.vcf.gz ]]; then
...
# Prepare chromosome names file if it doesn't exist
